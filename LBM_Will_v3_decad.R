...
for (i in 2:time_months) # a loop that simulates 1 to e.g. 120 months
{
  ...
  influx_mon[month] <- influx_mon_decad[i]
  # 1. lake mulurulu
  # black
  ...
  {
    leaghur$out    <- ( (leaghur$lev[i] - leaghur$mungolevel) * leaghur$size/1000 ) /2  #the outflow is the difference between the hypothetical reached lake level and the overflow level
    leaghur$lev[i] <- leaghur$mungolevel #+ (1000* leaghur$out/leaghur$size)
